...
types_of_teeth = ['all', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N']
selected_teeth = types_of_teeth[1]  # H
...
for x in range(angle, abs(angle), step):
    for y in range(angle, abs(angle), step):
        for z in range(angle, abs(angle), step):
            ...
            file = os.path.join(path, targetName, input, selected_teeth, ('x=' + str(x) + 'y=' + str(y) + 'z=' + str(z)))
            bpy.context.scene.render.filepath = file
            bpy.ops.render.opengl(write_still=True)

            bpy.ops.object.mode_set(mode='VERTEX_PAINT')

            file = os.path.join(path, targetName, output, selected_teeth,
            ...
            file = os.path.join(path, targetName, output_mask, selected_teeth,
                                ('x=' + str(x) + 'y=' + str(y) + 'z=' + str(z)))
            bpy.context.scene.render.filepath = file
            bpy.ops.render.opengl(write_still=True)
